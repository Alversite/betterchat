/**
 * Verbatim copy of Pisex's menus API, include/menus.h from
 * https://github.com/Pisex/cs2-menus release 1.9.1 - the utils.so installed
 * on the KILLHAUS servers (25.09.2026). Kept verbatim on purpose: BetterChat
 * builds a Menu and hands it to utils by reference, so struct Menu/Items must
 * have exactly their layout, and IMenusApi's virtuals exactly their order.
 *
 * Both sides use the old libstdc++ string ABI (_GLIBCXX_USE_CXX11_ABI=0),
 * checked on the binaries, so std::string/std::vector/std::function inside
 * Menu cross the plugin boundary safely. If utils is ever rebuilt with the
 * new ABI, this breaks - BetterChat only calls the first 10 virtuals, which
 * 1.9.1 kept in place (it appended the rest at the end).
 */
#pragma once
